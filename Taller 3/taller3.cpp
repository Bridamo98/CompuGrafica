...
// -------------------------------------------------------------------------
void KeyboardCbk( unsigned char key, int x, int y ) //lletra del teclado y pos del mouse
{
}
...
int main (int argc, char** argv) {
  ...
  win_data.Width = std::atoi( argv[ 1 ] ); // ancho y alto
  win_data.Height = std::atoi( argv[ 2 ] );
  ancho = win_data.Width;
  alto = win_data.Height;
  glutInit(&argc, argv);
  glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB);
  glutInitWindowPosition(100, 100);
  glutInitWindowSize(win_data.Width, win_data.Height);
  glutCreateWindow("taller 3");
  //glutDisplayFunc(myDisplay);
  glutKeyboardFunc( KeyboardCbk ); // teclado
